...
public class LeftHandOfEnemy : MonoBehaviour
{
    ...
    void Start() {
        this.gameObject.name = "__LeftHand";
    ...
    private void OnCollisionEnter2D(Collision2D collision) {
        ...
        if (oth.name == "LeftWall" ||
            ...
            if (oth.name == "Head") { // Наносим удар не получая повриждений.
            ...
            else { // Наносим удар не получая повриждений.
                ContactPoint2D[] contact = collision.contacts;
                Vector2 cont = contact[0].point;
                Vector2 move = VectorDirect(cont);
                rbObj.AddForce(move, ForceMode2D.Impulse);
